...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь
	walkingSpeed := math.Pow(meanSpeed(action, duration)*kmhInMsec, 2) / (height / cmInM)
	trainTimeInMin := duration * minInH
	return ((walkingCaloriesWeightMultiplier*weight + walkingSpeed*walkingSpeedHeightMultiplier*weight) * trainTimeInMin)
}
